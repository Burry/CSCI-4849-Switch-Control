...
function getSelectedItem() {
    ...
    else return '#' + selected.first().attr('id');
}
...
function selectNext() {
    ...
    if (selected == null) selectItem(buttonOrder[0]);
    else {
...
function clickSelectedItem() {
    ...
    if (whichButton != null) $(whichButton).click();
}
...
$(document).keydown(function(event) {
    ...
    else if (event.keyCode === 40) selectDown();
    else if (event.keyCode === 13) clickSelectedItem();
});
...
$('.calcButton').click(function(event) {
    ...
    if (digits.indexOf(buttonLabel) != -1) {
        ...
        if (!addingNumber) $('#number_input').val('');
...
function evaluateExpression(first, op, second) {
    output = 0;
    if (op == '+') output = parseInt(first) + parseInt(second);
    else if (op == '-') output = parseInt(first) - parseInt(second);
    else if (op == '*') output = parseInt(first) * parseInt(second);
    else if (op == '/') output = parseInt(first) / parseInt(second);
